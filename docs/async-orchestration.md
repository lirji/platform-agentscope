...
Reflexion 流使用 `POST /agent/reflexive/stream`，不创建持久任务；客户端断开即取消生成。
...
对应 counter。告警至少覆盖 heartbeat failure 增长、orphan failure 增长、running 长时间不归
零和 webhook dead outbox。上述自定义指标的标签中禁止出现 taskId、tenant、user、prompt、
result、webhook URL 或 token。
