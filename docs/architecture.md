...
6. 工具经 HTTP/MCP 调用 Java 服务，并传播 token 与 trace。
7. AgentScope 返回结果，应用层映射成稳定 DTO。
8. API 返回兼容 JSON/SSE，同时记录评测、审计、成本与追踪数据。
...
- Phase 2：DAG/Analyst/Reflexion/Voting/Chaining。
